...
fn main() {
    ...
    match command {
        ...
        Command::Key { user } => match Config::new(&CONF_PATH)
            .and_then(|conf| Ok(GithubClient::new(&conf)))
        ...
        Command::Pam => match env::var("PAM_USER") {
            Ok(user) => match Config::new(&CONF_PATH)
                .and_then(|conf| Ok(GithubClient::new(&conf)))
        ...
        Command::CleanUp => match Config::new(&CONF_PATH)
            .and_then(|conf| Ok(GithubClient::new(&conf)))
        ...
        Command::RateLimit => match Config::new(&CONF_PATH)
            .and_then(|conf| Ok(GithubClient::new(&conf)))
